...
namespace DLToolkit.Forms.Controls
{
    ...
    public class FlowObservableCollection<T> : ObservableCollection<T>
    {
        ...
        /// <summary>
        /// Adds the range. If T supports IComparable before adding checking will be perfomed 
        /// </summary>
		...
		public virtual void AddRange(IEnumerable<T> items)
        {
            ...
            bool compare = default(T) as IComparable != null;

            bool itemsAdded = false;
            foreach (var item in items)
            {
                if (!compare || !Items.Any(x => (IComparable)x == (IComparable)item))
                {
                    Items.Add(item);
                    itemsAdded = true;
                }
        ...
        public virtual void Repopulate(IList<T> items)
        {
            _disableOnCollectionChanged = true;
            Clear();
            foreach (var item in items)
